...
BEGIN
	...
	ELSE
		RETURN QUERY
		SELECT g.taskid, g.geom_utm AS geom FROM grids g
...
BEGIN
	...
	IF grid_type = 'MOCKUP' THEN
		IF NOT is_percentage THEN
			RAISE EXCEPTION 'Mockup grids are only generated by percentage';
		END IF;
	...
	ELSIF grid_type = 'ORIGINAL' THEN
		...
		SELECT utm_grids.taskid, shrink_geometry(utm_grids.geom, shrink_distance, is_percentage) AS geom
		FROM utm_grids;
	ELSE
		RAISE EXCEPTION 'Invalid grid type: %', grid_type;
	END IF;
END;
$$ LANGUAGE plpgsql PARALLEL SAFE;
...
BEGIN
    ...
    IF is_percentage THEN
		IF shrink_value = 100.0 THEN
			RETURN ST_SetSRID(ST_GeomFromText('POLYGON EMPTY'), ST_SRID(geom));
		END IF;
		...
		WHILE scale_factor IS NULL AND tries < max_tries THEN
			-- Dilate back the geometry by the scale factor
			scaled_geom := ST_Dilate(geom, scale_factor, tol => temp_tol, guess => sqrt(ST_Area(geom)) * _guess, safety => temp_safety);
			temp_tol = temp_tol * 2.0;
			temp_safety = temp_safety * 2.0;
			tries = tries + 1;
		END WHILE;
    ELSE
