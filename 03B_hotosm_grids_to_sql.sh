...
for project_id in $project_ids; do
    if [ -f "data/grid_${project_id}.geojson" ]; then
        ...
        if [ $current_project -ne 1 ]; then
            psql -h localhost -p 5432 -d hotosm -U postgres -c "ALTER TABLE hotosm_grids ALTER COLUMN project_id SET DEFAULT ${project_id};"
        fi
        
        # Transform GeoJSON to SQL
        ogr2ogr -f "PostgreSQL" PG:"host=localhost port=5432 dbname=hotosm user=postgres password=postgres" data/grid_${project_id}.geojson -nln hotosm_grids -nlt PROMOTE_TO_MULTI -lco GEOMETRY_NAME=geom -lco FID=gid -append -update
        ...
        if [ $current_project -eq 1 ]; then
            psql -h localhost -p 5432 -d hotosm -U postgres -c "ALTER TABLE hotosm_grids ADD COLUMN project_id INTEGER DEFAULT ${project_id};"
        fi
